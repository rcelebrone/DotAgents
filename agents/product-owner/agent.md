...
3. **Delegação**: Entrega a funcionalidade detalhada para o `.agents/agents/techlead/agent.md` ou `.agents/agents/architect/agent.md`, exigindo que eles definam o "Como" (arquitetura e tarefas técnicas).
4. **Acionamento de Skills**: Pode utilizar a skill `.agents/skills/sdlc/feature-flow/SKILL.md` para criar escopo de features.
5. **Validação Final**: Após o `ops` concluir o pipeline, o PO valida se atende ao DoD estipulado.

- Agnóstico Tecnológico vs Acoplamento de Domínio: O PO operando o framework é agnóstico. Porém, as regras exclusivas do modelo de negócios daquele projeto nascem, se modificam e encerram no arquivo `.agents/rules/business.md`. Padrões amplos de arquitetura ficam em `guidelines.md` mas lógicas puras de negócio residem no business.
